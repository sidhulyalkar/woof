...
import { CalendarHeart, Compass, HeartHandshake, Loader2, PawPrint, Sparkles, Users } from 'lucide-react';
    ...
    <div className="min-h-screen pb-24">
      ...
      <main className="mx-auto max-w-xl px-4 pb-8 pt-5">
        <section className="rounded-3xl border border-primary/15 bg-gradient-to-br from-primary/[0.1] via-card/90 to-secondary/[0.07] p-5">
          ...
          <h2 className="mt-1 text-2xl font-bold tracking-tight">Everybody can move the pack forward.</h2>
          <p className="mt-2 text-sm leading-relaxed text-muted-foreground">
            Pack challenges count appropriate shared actions, not raw distance. A senior dog, a cautious dog,
            and an adolescent trail rocket can all contribute without pretending their ideal weeks should look alike.
          </p>
        ...
        <section className="mt-6">
          ...
          {isLoading ? (
            ...
            <div className="surface-soft mt-3 rounded-2xl p-5 text-center">
              ...
              <p className="mt-1 text-sm text-muted-foreground">Your individual Adventure system still works normally.</p>
            </div>
          ) : (
                ...
                <article key={challenge.id} className="surface-soft rounded-3xl p-5">
                  <div className="flex items-start justify-between gap-3">
                    <div>
                      ...
                      <p className="mt-2 text-sm leading-relaxed text-muted-foreground">{challenge.description}</p>
                    </div>
                    {challenge.completed && (
                      <span className="rounded-full bg-primary/10 px-2.5 py-1 text-xs font-bold text-primary">Done</span>
                    )}
                  </div>
                  ...
                  <div className="mt-3 rounded-2xl bg-primary/[0.06] px-3 py-2 text-sm">
                    ...
                    {challenge.myContribution} meaningful {challenge.myContribution === 1 ? 'action' : 'actions'}
                  </div>
                </article>
              ))}
            </div>
          ...
          <div className="flex items-start gap-3">
            ...
            <div>
              ...
              <p className="mt-1 text-sm leading-relaxed text-muted-foreground">
                Pack does not reward forcing interactions. Parallel walks, extra distance, ending early, or skipping a
                crowded event can all be better choices for a particular dog.
              </p>
