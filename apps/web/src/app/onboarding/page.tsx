...
export default function OnboardingPage() {
  ...
  useEffect(() => {
    ...
    if (isAuthenticated && authUser && currentStep === 1 && !ownerData) {
      setOwnerData({
  ...
  const attachPhoto = async (pair: DurablePair, photoFile: File | null) => {
    ...
    try {
      ...
      toast.warning("Your pet is ready, but the photo could not be attached. You can add it later.")
    }
  }
    ...
    try {
      if (!useAuthStore.getState().isAuthenticated) {
        if (!ownerData?.password) {
          throw new Error("Your account password is missing. Return to the first step and try again.")
        }
  ...
  const finishFirstAdventure = async (selections: FirstAdventureSelections, skipAll = false) => {
    ...
    const responses = buildFirstAdventureResponses(
      durablePair.petId,
      selections,
      skipAll
    )
      ...
      <main id="main-content" className="mx-auto max-w-lg px-4 py-8">
        ...
        {currentStep === 3 && durablePair && (
          <FirstAdventureStep
            ...
            onSkipAll={() =>
              void finishFirstAdventure(emptyFirstAdventureSelections(), true)
            }
          />
        )}
