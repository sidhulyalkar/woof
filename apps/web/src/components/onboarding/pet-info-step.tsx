  ...
  return (
    <form onSubmit={handleSubmit} className="space-y-6">
      <div className="space-y-2">
        ...
        <h1 className="text-3xl font-bold tracking-tight text-balance">A few facts, then we can do something together</h1>
        <p className="text-sm leading-relaxed text-muted-foreground text-pretty">
          Woof only needs durable basics here. Preferences, comfort, and what actually works are better learned from real Adventures than from asking you to describe your pet all at once.
        </p>
      </div>
      ...
      <Card className="glass rounded-2xl p-5">
        <div className="flex items-center gap-4">
          ...
          <div className="min-w-0 flex-1">
            <Label htmlFor="pet-photo" className="inline-flex min-h-11 cursor-pointer items-center gap-2 rounded-xl border border-border px-3 py-2 text-sm font-semibold hover:border-primary/40 hover:bg-primary/5">
            ...
            <p className="mt-2 text-xs leading-relaxed text-muted-foreground">
              Optional. We create your pet first, then attach the photo. A failed upload can never block your first Adventure.
            </p>
            {formData.photoFile && (
              <Button type="button" variant="ghost" size="sm" className="mt-1 h-auto min-h-0 px-0 text-muted-foreground hover:text-destructive" onClick={removePhoto} disabled={isLoading}>
      ...
      <div className="space-y-4">
        ...
        <div className="grid gap-4 sm:grid-cols-2">
          <div className="space-y-2">
            ...
            <Select value={formData.species} onValueChange={(value) => setFormData({ ...formData, species: value })} disabled={isLoading}>
          ...
          <div className="space-y-2">
            ...
            <Input
              ...
              disabled={isLoading}
            />
            <p className="text-xs text-muted-foreground">Life stage can change which activities are a sensible starting point.</p>
          </div>
        </div>

        <div className="space-y-2">
          <Label htmlFor="breed">Breed or mix <span className="text-muted-foreground">(optional)</span></Label>
          <Input
            id="breed"
            ...
            disabled={isLoading}
          />
          <p className="text-xs text-muted-foreground">Supporting context only. Woof learns this individual from their own outcomes rather than treating breed as destiny.</p>
        </div>
      </div>
