...
export function PetInfoStep({ onComplete, initialData }: PetInfoStepProps) {
  ...
  const isValid = Boolean(formData.name && formData.species && formData.breed && formData.birthdate)

  return (
    <form onSubmit={handleSubmit} className="space-y-6">
      <div className="space-y-2">
        <p className="eyebrow">Pet profile</p>
        <h1 className="text-3xl font-bold tracking-tight text-balance">Tell Woof who we&apos;re matching for</h1>
        <p className="text-sm leading-relaxed text-muted-foreground text-pretty">
          Start with durable profile facts and a few social traits. Health records belong in an explicit private flow, not a casual social onboarding form.
        </p>
      </div>
        ...
        <div className="flex items-center gap-4">
          ...
          <div className="min-w-0 flex-1">
            ...
            <p className="mt-2 text-xs leading-relaxed text-muted-foreground">
              Optional. The file is uploaded only after your account exists, so failed registration never leaves an orphaned image.
            </p>
            {formData.photoFile && (
              <Button type="button" variant="ghost" size="sm" className="mt-1 h-auto min-h-0 px-0 text-muted-foreground hover:text-destructive" onClick={removePhoto}>
      ...
      <div className="space-y-4">
        ...
        <div className="space-y-2">
          ...
          <Input
            ...
            required
          />
          <p className="text-xs text-muted-foreground">Breed is only a light supporting signal in compatibility, never the primary decision rule.</p>
        </div>

        <fieldset className="space-y-3">
          <legend className="text-sm font-medium">Social temperament</legend>
          <p className="text-xs text-muted-foreground">Choose the traits that are consistently true today. These can change over time.</p>
