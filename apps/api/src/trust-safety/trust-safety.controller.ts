...
export class TrustSafetyController {
  ...
  blockUser(@Request() req: any, @Body() dto: BlockUserDto) {
    return this.trustSafetyService.blockUser(req.user.sub, dto);
  }

  @Delete('blocks/:userId')
  @ApiOperation({ summary: 'Remove a block' })
  unblockUser(@Request() req: any, @Param('userId') blockedUserId: string) {
  ...
  reportUser(@Request() req: any, @Body() dto: ReportUserDto) {
