...
export const REQUEST_DURATION_BUCKETS_MS = [25, 50, 100, 250, 500, 750, 1000, 1500, 2500, 5000] as const;
...
export class OperationalMetricsService {
  ...
  prometheus() {
    ...
    const lines = [
      ...
      '# HELP woof_http_request_duration_ms Request duration histogram by operation and status class.',
    ...
    for (const metric of this.requests.values()) {
      ...
      lines.push(`woof_http_request_duration_ms_bucket{${labels},le="+Inf"} ${metric.count}`);
