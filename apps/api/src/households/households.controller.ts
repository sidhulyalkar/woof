...
export class HouseholdsController {
  ...
  update(@Request() req: any, @Param('id') id: string, @Body() dto: UpdateHouseholdDto) {
    return this.households.update(req.user.sub, id, dto);
  }

  @Post(':id/pets/:petId')
  @ApiOperation({ summary: 'Add one owned pet to a managed household' })
  addPet(@Request() req: any, @Param('id') id: string, @Param('petId') petId: string) {
  ...
  removePet(@Request() req: any, @Param('id') id: string, @Param('petId') petId: string) {
