...
import type { CorrectDailySignalsDto, DailySignalsCurrentQueryDto } from './dto/daily-signals-correction.dto';
...
function structuredPayloadHash(signals: DailySignalsAnswers) {
  ...
  return createHash('sha256').update(JSON.stringify({ signals: ordered })).digest('hex');
}
...
export class DailySignalsCorrectionService {
    ...
    const corrections = rows
      ...
      .map((row) => {
        if (!row.pet_id || row.outcome?.note !== undefined) {
          throw new ConflictException('Canonical Daily Signals correction privacy shape is invalid');
        }
  ...
  private async reconcileChain(userId: string, chain: DailySignalsChain) {
    ...
    for (const row of rows) {
      ...
      const eventMap = byEvent.get(row.source_event_id) ?? new Map<SignalDimension, ProjectionRow>();
    ...
    for (const field of DAILY_SIGNAL_FIELDS) {
      ...
      if (!row) {
        throw new ConflictException('Canonical Daily Signals root projection could not be repaired');
      }
    ...
    for (const correction of chain.events.slice(1)) {
      for (const field of DAILY_SIGNAL_FIELDS) {
        ...
        if (choice !== undefined && choice !== 'UNSURE') {
          ...
          const eventMap =
            byEvent.get(correction.id) ?? new Map<SignalDimension, ProjectionRow>();
