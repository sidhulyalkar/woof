...
export class NudgesController {
  ...
  async getUserNudges(@Request() req: any) {
  ...
  async dismissNudge(@Param('id') id: string, @Request() req: any) {
  ...
  async acceptNudge(@Param('id') id: string, @Request() req: any) {
  ...
  async checkChatActivity(
    ...
    return this.nudgesService.checkChatActivityNudges(
      conversationId,
      req.user.sub,
    );
  }
